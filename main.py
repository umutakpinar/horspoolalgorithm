def black_match_table(searchPattern="TOOTH"): #aranacak kelime bu şekilde parametre olarak verilmeli değer verilmezse TOOOTH
    pattern = list(searchPattern)
    length = len(pattern) #patternin boyutunu aldık
    skip = [i for i in range(256)] #ASCII tablsoundaki karakterlerin hepsi geçerli olsun diye 256 elemanlı bir dizi oluşturduk
    ...
    for i in pattern:
        if i != pattern[len(pattern)-1]: #patternin son elemanı hariç hepsinin değerlerini hesapladık (III. kural)
            skip[ord(i)] = length - pattern.index(i) - 1 #256 karakterli dizi oluşturduğumuz için ASCII'ye dönüştürdük ord() metodu ile her birinin value'larını hesaplayıp skip tablosundaki ilgili indexlere gönderdik
